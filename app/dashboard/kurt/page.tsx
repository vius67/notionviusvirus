...
export default function KurtPage() {
  const { user } = useAuth()
  ...
  const addHw = async () => {
    ...
    if (data) setHw(p => [data, ...p].sort((a, b) => {
  ...
  const toggleHw = async (id: string, current: boolean) => {
    setHw(p => p.map(h => h.id === id ? { ...h, completed: !current } : h))
  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div className="fade-up" style={{ marginBottom: 28 }}>
        <p className="page-eyebrow">Tutoring Centre</p>
        <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', gap: 16, flexWrap: 'wrap' }}>
          <div>
            <h1 style={{ fontSize: isMobile ? 26 : 34, fontWeight: 780, letterSpacing: '-0.045em', color: 'var(--text-primary)', lineHeight: 1.1 }}>Kurt</h1>
            <p style={{ fontSize: 13.5, color: 'var(--text-muted)', marginTop: 4 }}>Homework & past paper tracker for your tutoring sessions</p>
          </div>
          {/* Quick stats chips */}
        ...
        <div className="fade-up" style={{ display: 'flex', gap: 6, overflowX: 'auto', paddingBottom: 4, marginBottom: 24, animationDelay: '40ms' }}>
          {['all', ...allSubjects].map(s => (
            <button key={s} onClick={() => setSubjFilter(s)} style={{
              padding: '5px 14px', borderRadius: 999, border: 'none', cursor: 'pointer', whiteSpace: 'nowrap',
        ...
        <div className="glass-card fade-up" style={{ padding: 0, overflow: 'hidden', animationDelay: '60ms' }}>
          ...
          <div style={{ padding: '18px 20px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <div style={{ fontSize: 14.5, fontWeight: 660, color: 'var(--text-primary)' }}>Homework</div>
                ...
                >
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
                      <span style={{ fontSize: 13.5, fontWeight: 520, color: item.completed ? 'var(--text-muted)' : 'var(--text-primary)', textDecoration: item.completed ? 'line-through' : 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: '100%' }}>{item.title}</span>
                    </div>
        ...
        <div className="glass-card fade-up" style={{ marginTop: 20, padding: '20px 24px', animationDelay: '140ms' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12 }}>
            {allSubjects.map(subj => {
              ...
              const subjAvg = subjScored.length
                ? Math.round(subjScored.reduce((a, p) => a + (p.score! / p.max_score!) * 100, 0) / subjScored.length)
                : null
